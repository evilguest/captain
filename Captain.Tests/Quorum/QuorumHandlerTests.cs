...
namespace Captain.Tests.Quorum
{
    ...
    public class QuorumHandlerTests
    {
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => QuorumHandler.Create(42, 0, 0));
            Assert.Throws<ArgumentOutOfRangeException>(() => QuorumHandler.Create(42, 32, 2));
            Assert.Throws<ArgumentOutOfRangeException>(() => QuorumHandler.Create(42, 15, 16));
            Assert.Throws<ArgumentOutOfRangeException>(() => QuorumHandler.Create(42, 3, 0));
        }
        [Theory]
        [InlineData(42, 1)]
        [InlineData(42, 3)]
        ...
        {
            var q = QuorumHandler.Create(seed, machineCount, 1);
        ...
        {
            var q = QuorumHandler.Create(42, 3, 2);
            var rs = TestHelper.GetTopupThenWithdrawals(10, 42, 10, TimeSpan.FromSeconds(1)).ToList();
            var sts = Analyzer.SingleMachineProcess(rs).ToArray();
